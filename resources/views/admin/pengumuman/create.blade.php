...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Tambah Pengumuman Baru</h1>
            <p class="text-black mt-1">Isi form di bawah untuk menambahkan pengumuman baru</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <form method="POST" action="{{ route('admin.pengumuman.store') }}" class="p-6 space-y-6">
            @csrf

            <!-- Judul Field -->
            <div>
                <label for="judul" class="block text-sm font-semibold text-black mb-2">
                    Judul Pengumuman <span class="text-black">*</span>
                </label>
                <div class="relative">
            ...
            <div>
                <label for="isi" class="block text-sm font-semibold text-black mb-2">
                    Isi Pengumuman <span class="text-black">*</span>
                </label>
            ...
            <!-- Tanggal Field -->
            <div>
                <label for="tanggal" class="block text-sm font-semibold text-black mb-2">
                    Tanggal Pengumuman <span class="text-black">*</span>
                </label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}" required
                        class="pl-10 block w-full rounded-lg border @error('tanggal') border-yellow-400 @else border-gray-300 @enderror focus:ring-yellow-400 focus:border-yellow-400 shadow-sm py-2.5 text-sm @error('tanggal') ring-2 ring-yellow-200 @enderror">
                </div>
                @error('tanggal')
                    <p class="mt-2 text-sm text-black flex items-center">
                        <svg class="w-4 h-4 mr-1.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
