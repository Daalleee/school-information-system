...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
            ...
            <p class="text-black mt-1">Perbarui informasi foto galeri</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <form method="POST" action="{{ route('admin.galeri.update', $galeri) }}" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <label for="judul" class="block text-sm font-semibold text-black mb-2">
                    Judul Foto <span class="text-black">*</span>
                </label>
                <div class="relative">
            ...
            <div>
                <label class="block text-sm font-semibold text-black mb-2">
                    Gambar <span class="text-white font-normal">(opsional)</span>
                </label>
                <div class="flex items-start space-x-6">
                    <div class="flex-shrink-0">
                        <div id="gambarPreview" class="h-48 w-64 rounded-lg bg-white border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                            @if($galeri->gambar)
                                <img id="previewImg" src="{{ asset('storage/' . $galeri->gambar) }}" alt="{{ $galeri->judul }}"
                                    class="h-full w-full object-cover rounded-lg">
                            @else
                                <svg id="previewPlaceholder" class="w-16 h-16 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="flex-1">
                        <div class="flex items-center justify-center w-full">
                            <label for="gambar"
                                class="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-white hover:bg-white transition-colors">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <svg class="w-10 h-10 mb-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                    </svg>
                                    <p class="mb-2 text-sm text-black"><span class="font-semibold">Klik untuk upload</span> atau drag & drop</p>
                                    <p class="text-xs text-black">PNG, JPG, JPEG, GIF (Maks. 2MB)</p>
                                    @if($galeri->gambar)
                                        <p class="mt-2 text-xs text-black">Gambar saat ini akan diganti jika upload baru.</p>
                                    @endif
                                </div>
            ...
            <!-- Kategori Field -->
            <div>
                <label for="kategori" class="block text-sm font-semibold text-black mb-2">
                    Kategori
                </label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    ...
                    <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $galeri->kategori) }}"
                        ...
                        class="pl-10 block w-full rounded-lg border @error('kategori') border-yellow-400 @else border-gray-300 @enderror focus:ring-yellow-400 focus:border-yellow-400 shadow-sm py-2.5 text-sm @error('kategori') ring-2 ring-yellow-200 @enderror">
                </div>
                @error('kategori')
                    <p class="mt-2 text-sm text-black flex items-center">
                        <svg class="w-4 h-4 mr-1.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="bg-white rounded-lg p-4 border border-gray-200">
                <h3 class="text-sm font-semibold text-black mb-3">Informasi Foto</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                        <span class="text-black">Dibuat pada:</span>
                        <span class="ml-2 text-gray-900 font-medium">{{ $galeri->created_at->format('d M Y, H:i') }}</span>
                    </div>
                    <div>
                        <span class="text-black">Terakhir diperbarui:</span>
                        <span class="ml-2 text-gray-900 font-medium">{{ $galeri->updated_at->format('d M Y, H:i') }}</span>
                    </div>
                </div>
            </div>
