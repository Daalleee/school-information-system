...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <form method="GET" action="{{ route('admin.kontak.index') }}" class="space-y-4">
            <div class="flex flex-col sm:flex-row gap-4">
                <!-- Search Input -->
                <div class="flex-1">
                    <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Cari Pesan</label>
                    <div class="relative">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($pesan as $p)
                        <tr class="hover:bg-gray-50 transition-colors {{ !$p->is_read ? 'bg-orange-50' : '' }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-600 line-clamp-2 max-w-xs">{{ Str::limit($p->pesan, 100) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($p->is_read)
        ...
        <div class="md:hidden divide-y divide-gray-200">
            @forelse($pesan as $p)
                <div class="p-4 space-y-3 {{ !$p->is_read ? 'bg-orange-50' : '' }}">
                    <div class="flex items-start justify-between">
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-600 mt-1 line-clamp-2">{{ Str::limit($p->pesan, 80) }}</p>
                            </div>
                        </div>
                        <div>
                            @if($p->is_read)
            ...
            @empty
                <div class="p-8 text-center">
                    <p class="text-sm text-gray-500">Tidak ada pesan masuk.</p>
                </div>
            @endforelse
        </div>
